<?php
// DIMF_2007.php - État des biens détenus dans le cadre de la concession (avec FPDF)
session_start();
...
class PDF_DIMF extends FPDF {
    ...
    function Header() {
        ...
        $this->SetFont('Arial','B',13); $this->SetX(8);
        $this->Cell(0,7,self::u($this->codeDimf.'  -  '.$this->titreDimf),0,1,'L');
        $this->SetFont('Arial','',8); $this->SetX(8);
    ...
    function TableHeader($cols) {
        $this->SetFont('Arial','B',8); $this->SetFillColor(248,250,252); $this->SetTextColor(30,41,59); $this->SetDrawColor(226,232,240);
        foreach ($cols as $col) { $align = isset($col['align'])?$col['align']:'L'; $this->Cell($col['w'],6,self::u($col['label']),1,0,$align,true); }
        $this->Ln();
    }
    function TableRow($cols,$data,$style='') {
        ...
        else { $this->SetFillColor(255,255,255); $this->SetFont('Arial','',7.5); }
        $this->SetTextColor(15,23,42); $this->SetDrawColor(226,232,240);
        foreach ($cols as $i=>$col) { $val = isset($data[$i])?$data[$i]:''; $align = isset($col['align'])?$col['align']:'L'; $this->Cell($col['w'],5.5,self::u($val),1,0,$align,$fill); }
        $this->Ln();
    }
...
// Traitement formulaire (ajout, modification, suppression)
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action'])) {
    ...
    try {
        $pdo->exec("CREATE TABLE IF NOT EXISTS biens_concession (id INT AUTO_INCREMENT PRIMARY KEY, exercice INT NOT NULL, libelle VARCHAR(255) NOT NULL, valeur_inventaire DECIMAL(15,2) DEFAULT 0, concessionnaire_nom VARCHAR(200), valeur_declaree DECIMAL(15,2) DEFAULT 0, date_acquisition DATE, duree_concession INT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)");
        if ($_POST['action']==='add') {
            ...
            $stmt->execute([':exercice'=>$exercice, ':libelle'=>$_POST['libelle']??'', ':valeur_inventaire'=>$_POST['valeur_inventaire']??0, ':concessionnaire'=>$_POST['concessionnaire_nom']??'', ':valeur_declaree'=>$_POST['valeur_declaree']??0, ':date_acquisition'=>$date_acquisition, ':duree'=>$duree_concession]);
            ...
            $stmt->execute([':id'=>(int)$_POST['id'], ':exercice'=>$exercice, ':libelle'=>$_POST['libelle']??'', ':valeur_inventaire'=>$_POST['valeur_inventaire']??0, ':concessionnaire'=>$_POST['concessionnaire_nom']??'', ':valeur_declaree'=>$_POST['valeur_declaree']??0, ':date_acquisition'=>$date_acquisition, ':duree'=>$duree_concession]);
            $message="Bien modifié !"; $message_type="success";
        }
    } catch(PDOException $e){ $message="Erreur : ".$e->getMessage(); $message_type="error"; }
    header("Location: DIMF_2007.php?exercice=$exercice&type_periode=$type_periode".($type_periode=='mensuel'?"&mois=$mois":($type_periode=='trimestre'?"&trimestre=$trimestre":($type_periode=='semestre'?"&semestre=$semestre":"")))."&msg=".urlencode($message)."&msg_type=$message_type");
    exit;
}
if(isset($_GET['msg'])){ $message=$_GET['msg']; $message_type=$_GET['msg_type']??'success'; }

// Récupération biens
$biens_concession = []; $total_valeur_inventaire=0; $total_valeur_declaree=0;
try {
    $stmt = $pdo->prepare("SELECT * FROM biens_concession WHERE exercice = :exercice ORDER BY id");
    $stmt->execute([':exercice'=>$exercice]); $biens_concession=$stmt->fetchAll();
    foreach($biens_concession as $b){ $total_valeur_inventaire+=$b['valeur_inventaire']; $total_valeur_declaree+=$b['valeur_declaree']; }
} catch(PDOException $e){}

$edit_bien = null;
if(isset($_GET['edit']) && is_numeric($_GET['edit'])){
    try{ $stmt=$pdo->prepare("SELECT * FROM biens_concession WHERE id=:id AND exercice=:exercice"); $stmt->execute([':id'=>$_GET['edit'],':exercice'=>$exercice]); $edit_bien=$stmt->fetch(); } catch(PDOException $e){}
}
...
if($format==='pdf'){
    ...
    $pdf->codeDimf='DIMF_2007'; $pdf->titreDimf='Biens en concession'; $pdf->nomSfd=$_SESSION['nom_sfd']??'SFD'; $pdf->periode=$lib_periode; $pdf->exercice=$exercice;
    $pdf->SetMargins(8,35,8); $pdf->SetAutoPageBreak(true,14); $pdf->AddPage();
    $cols = [['label'=>'Libellé','w'=>60],['label'=>'Valeur inventaire','w'=>45,'align'=>'R'],['label'=>'Concessionnaire','w'=>50],['label'=>'Valeur déclarée','w'=>45,'align'=>'R'],['label'=>'Date acquisition','w'=>35,'align'=>'C'],['label'=>'Durée (ans)','w'=>30,'align'=>'C']];
    $pdf->SectionTitle('Biens en concession');
    $pdf->TableHeader($cols);
    foreach($biens_concession as $b){
        $pdf->TableRow($cols,[
            PDF_DIMF::u($b['libelle']),
            PDF_DIMF::montant($b['valeur_inventaire']),
            PDF_DIMF::u($b['concessionnaire_nom']?:'-'),
            PDF_DIMF::montant($b['valeur_declaree']),
            $b['date_acquisition']?date('d/m/Y',strtotime($b['date_acquisition'])):'-',
            $b['duree_concession']?$b['duree_concession'].' ans':'-'
        ]);
    }
    $pdf->TableRow($cols,['TOTAL',PDF_DIMF::montant($total_valeur_inventaire),'',PDF_DIMF::montant($total_valeur_declaree),'',''],'total');
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;} body{font-family:'Inter',sans-serif;background:#f1f5f9;padding:24px;}
        ...
        td{padding:10px 16px;border-bottom:1px solid #f1f5f9;} .text-right{text-align:right;} .total-row{background:#f0fdf4;font-weight:700;}
        ...
        .action-buttons{display:flex;gap:8px;} .btn-warning{background:#f59e0b;color:white;padding:4px 12px;border-radius:20px;text-decoration:none;font-size:0.75rem;}
...
<div class="dashboard">
    <div class="page-header">
        <div class="header-left"><h1><i class="fas fa-landmark"></i> DIMF_2007 - BIENS EN CONCESSION</h1><div class="subtitle">République de Côte d'Ivoire – DGTCP / DSFD</div><div class="badge">SICS-BCEAO</div></div>
    ...
    <div class="card">
        ...
        <form method="post">
            ...
            <div class="filters-row" style="margin-bottom:0;">
                ...
                <div class="filter-item"><label>Valeur inventaire (FCFA)</label><input type="number" name="valeur_inventaire" value="<?= $edit_bien?(int)$edit_bien['valeur_inventaire']:0 ?>"></div>
                <div class="filter-item"><label>Concessionnaire</label><input type="text" name="concessionnaire_nom" value="<?= $edit_bien?htmlspecialchars($edit_bien['concessionnaire_nom']):'' ?>"></div>
                <div class="filter-item"><label>Valeur déclarée (FCFA)</label><input type="number" name="valeur_declaree" value="<?= $edit_bien?(int)$edit_bien['valeur_declaree']:0 ?>"></div>
                <div class="filter-item"><label>Date acquisition</label><input type="date" name="date_acquisition" value="<?= $edit_bien&&$edit_bien['date_acquisition']?date('Y-m-d',strtotime($edit_bien['date_acquisition'])):'' ?>"></div>
                <div class="filter-item"><label>Durée (années)</label><input type="number" name="duree_concession" value="<?= $edit_bien&&$edit_bien['duree_concession']!==null?(int)$edit_bien['duree_concession']:'' ?>"></div>
                <div class="filter-item"><button type="submit" class="btn-apply"><?= $edit_bien?'Mettre à jour':'Ajouter' ?></button></div>
                <?php if($edit_bien): ?><div class="filter-item"><a href="DIMF_2007.php?exercice=<?= $exercice ?>&type_periode=<?= $type_periode ?><?= $type_periode=='mensuel'?"&mois=$mois":($type_periode=='trimestre'?"&trimestre=$trimestre":($type_periode=='semestre'?"&semestre=$semestre":"")) ?>" class="btn-warning">Annuler</a></div><?php endif; ?>
            </div>
        </form>
    </div>
    <div class="card">
        <div class="card-header"><i class="fas fa-list-ul"></i> LISTE DES BIENS</div>
        <?php if(empty($biens_concession)): ?><div class="info-box">Aucun bien enregistré.</div><?php else: ?>
        <div class="table-wrapper"><table>
            <thead><th>Libellé</th><th class="text-right">Valeur inventaire</th><th>Concessionnaire</th><th class="text-right">Valeur déclarée</th><th>Date acquisition</th><th>Durée</th><th>Actions</th></thead>
            <tbody><?php foreach($biens_concession as $b): ?>
                <tr><td><?= htmlspecialchars($b['libelle']) ?></td><td class="text-right"><?= number_format($b['valeur_inventaire'],0,',',' ') ?></td><td><?= htmlspecialchars($b['concessionnaire_nom']?:'-') ?></td><td class="text-right"><?= number_format($b['valeur_declaree'],0,',',' ') ?></td><td><?= $b['date_acquisition']?date('d/m/Y',strtotime($b['date_acquisition'])):'-' ?></td><td><?= $b['duree_concession']?$b['duree_concession'].' ans':'-' ?></td>
            ...
            <?php endforeach; ?>
            <tr class="total-row"><td><strong>TOTAL</strong></td><td class="text-right"><strong><?= number_format($total_valeur_inventaire,0,',',' ') ?></strong></td><td></td><td class="text-right"><strong><?= number_format($total_valeur_declaree,0,',',' ') ?></strong></td><td colspan="3"></td></tr>
            </tbody>
        </table></div><?php endif; ?>
    </div>
...
<script>
    ...
    function exporterExcel(){ const wb=XLSX.utils.book_new(); const data=[['DIMF_2007 - BIENS EN CONCESSION'],['Exercice','<?= $exercice ?>','Type','<?= $type_periode ?>'],[],['Libellé','Valeur inventaire','Concessionnaire','Valeur déclarée','Date acquisition','Durée']];
        <?php foreach($biens_concession as $b): ?> data.push(['<?= addslashes($b['libelle']) ?>',<?= $b['valeur_inventaire'] ?>,'<?= addslashes($b['concessionnaire_nom']?:'-') ?>',<?= $b['valeur_declaree'] ?>,'<?= $b['date_acquisition']?date('d/m/Y',strtotime($b['date_acquisition'])):'-' ?>','<?= $b['duree_concession']??'-' ?>']); <?php endforeach; ?>
        data.push(['TOTAL',<?= $total_valeur_inventaire ?>,'',<?= $total_valeur_declaree ?>,'','']); const ws=XLSX.utils.aoa_to_sheet(data); XLSX.utils.book_append_sheet(wb,ws,"BIENS_CONCESSION"); XLSX.writeFile(wb,'DIMF_2007_<?= $exercice ?>_<?= $type_periode ?>.xlsx'); }
</script>
</body>
</html>