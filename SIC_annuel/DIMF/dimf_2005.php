<?php
// DIMF_2005.php - Tableau des emplois et ressources (avec FPDF intégré)
...
// ============================================================
// CALCULS MÉTIER (EMPLOIS, RESSOURCES, VARIATIONS)
// ============================================================
...
$actif_net_prec = 0; // (non utilisé dans l'affichage, mais conservé)

// Fonctions d'affichage HTML des variations
...
if ($format === 'pdf') {
    ...
    // Colonnes : 6 colonnes pour le tableau des variations
    $cols = [
        ['label' => 'CODE',            'w' => 25],
        ['label' => 'LIBELLÉS',        'w' => 90],
    ...
    $pdf->MultiCell(0, 5, PDF_DIMF::u(
        ...
        "• Évolution du portefeuille de crédits : " . PDF_DIMF::montant(abs($variation_absolue)) . " (" . number_format($variation_pct,2) . "%)\n" .
        "• Évolution des fonds propres : " . PDF_DIMF::montant(abs($fonds_propres_cours - $fonds_propres_prec))
    ));
...
<div class="dashboard">
    ...
    <div class="card">
        ...
        <div class="table-wrapper">
            <table>
                <thead><th>CODE</th><th>LIBELLÉS</th><th class="text-right">Période précédente (N-1)</th><th class="text-right">Période en cours (N)</th><th class="text-right">Variation absolue</th><th class="text-right">Variation %</th></thead>
                <tbody>
...
<script>
    ...
    function appliquerFiltres() {
        ...
        const type = document.getElementById('typePeriodeSelect').value;
        let url = 'DIMF_2005.php?exercice='+exercice+'&type_periode='+type;
