...
<script>
    ...
    function appliquerFiltres() {
        ...
        const type = document.getElementById('typePeriodeSelect').value;
        let url = 'DIMF_2900.php?exercice=' + exercice + '&type_periode=' + type;
    ...
    function exporterExcel() {
        ...
        // Onglet ACTIF
        let dataActif = [
            ['DIMF_2900 - BILAN CONSOLIDE'],
            ['Periode : <?= addslashes($lib_periode) ?>'],
            [],
            ['CODE POSTE', 'ACTIF', 'Montant (FCFA)'],
            ['010', 'CAISSE', <?= $caisse ?>],
            ['014', 'CREANCES SUR LES INSTITUTIONS FINANCIERES', ''],
            ['015', '  - A vue', <?= $creances_vue ?>],
            ['030', 'CREANCES SUR LES MEMBRES OU BENEFICIAIRES', ''],
            ['037', '  - Credits ordinaires', <?= $credits_ordinaires ?>],
            ['051', 'CREDIT-BAIL ET OPERATIONS ASSIMILEES', <?= $credit_bail ?>],
            ['110', 'IMMOBILISATIONS FINANCIERES', <?= $immobilisations_financieres ?>],
            ['140', 'IMMOBILISATIONS INCORPORELLES', <?= $immobilisations_incorporelles ?>],
            ['145', 'IMMOBILISATIONS CORPORELLES', <?= $immobilisations_corporelles ?>],
            ['250', 'TOTAL ACTIF', <?= $total_actif ?>]
        ];
        ...
        let dataPassif = [
            ...
            ['Periode : <?= addslashes($lib_periode) ?>'],
            [],
            ['CODE POSTE', 'PASSIF', 'Montant (FCFA)'],
            ['300', 'DETTES A L\'EGARD DES INSTITUTIONS FINANCIERES', ''],
            ['310', '  - A vue', <?= $dettes_vue ?>],
            ['330', 'DETTES A L\'EGARD DES MEMBRES OU BENEFICIAIRES', ''],
            ['331', '  - Comptes d\'epargne a vue', <?= $epargne_vue ?>],
            ['332', '  - Comptes d\'epargne a terme', <?= $epargne_terme ?>],
            ['360', 'PROVISIONS POUR RISQUES ET CHARGES', <?= $provisions_risques ?>],
            ['362', 'EMPRUNTS ET TITRES EMIS SUBORDONNES', <?= $emprunts_subordonnes ?>],
            ['370', 'SUBVENTIONS D\'INVESTISSEMENT', <?= $subventions_investissement ?>],
            ['380', 'CAPITAL', <?= $capital ?>],
            ['400', 'REPORT A NOUVEAU', <?= $report_nouveau ?>],
            ['420', 'EXCEDENT OU DEFICIT DE L\'EXERCICE', <?= $resultat_exercice ?>],
            ['450', 'TOTAL PASSIF', <?= $total_passif ?>]
        ];
        XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(dataPassif), "PASSIF");
        
        // Onglet HORS BILAN
        let dataHB = [
            ['DIMF_2900 - HORS BILAN CONSOLIDE'],
            ['Periode : <?= addslashes($lib_periode) ?>'],
            [],
            ['CODE POSTE', 'HORS BILAN', 'Montant (FCFA)'],
            ['', 'ENGAGEMENTS DONNES', <?= $engagements_donnes ?>],
            ['465', '  - En faveur des institutions financieres', <?= $engagements_financement_inst ?>],
            ['470', '  - En faveur des membres, beneficiaires ou clients', <?= $engagements_financement_membres ?>],
            ['', '  - Engagements de garantie - D\'ordre des membres', <?= $engagements_garantie_membres ?>],
            ['', 'ENGAGEMENTS RECUS', <?= $engagements_recus ?>],
            ['', 'TOTAL HORS BILAN', <?= $total_hors_bilan ?>]
        ];
        XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(dataHB), "HORS_BILAN");
        
        XLSX.writeFile(wb, 'DIMF_2900_<?= $exercice ?>.xlsx');
    }
