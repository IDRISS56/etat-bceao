...
// Sauvegarde des modifications (POST)
...
<div class="dashboard">
    ...
    <form method="post" action="">
        ...
        <!-- Conserver les paramètres de période pour le POST d'enregistrement -->
        <input type="hidden" name="exercice" value="<?= $exercice ?>">
        <input type="hidden" name="type_periode" value="<?= $type_periode ?>">
        <input type="hidden" name="mois" value="<?= $mois ?>">
        <input type="hidden" name="trimestre" value="<?= $trimestre ?>">
...
<script>
    // Mise à jour dynamique du select de période (filtres et formulaire)
    function updateDynamicSelect() {
        ...
        if (type === 'mensuel') {
            ...
            for (let m = 1; m <= 12; m++) {
                ...
                formHtml += `<option value="${m}" ${selected}>${String(m).padStart(2,'0')} - ${monthName}</option>`;
            }
            html += '</select>';
            formHtml += '</select></div>';
            ...
            for (let t = 1; t <= 4; t++) {
                ...
                formHtml += `<option value="${t}" ${selected}>${t}${t === 1 ? 'er' : 'eme'} Trimestre</option>`;
            }
            html += '</select>';
            formHtml += '</select></div>';
            ...
            for (let s = 1; s <= 2; s++) {
                ...
                formHtml += `<option value="${s}" ${selected}>${s}${s === 1 ? 'er' : 'e'} semestre</option>`;
            }
            html += '</select>';
            formHtml += '</select></div>';
        } else {
    ...
    // La fonction appliquerFiltres n'est plus nécessaire car le formulaire de filtres se soumet en POST.
    // On conserve updateDynamicSelect pour les changements de type période.
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Synchronisation du select de période dans le formulaire d'enregistrement
        const formTypePeriode = document.getElementById('formTypePeriode');
        if (formTypePeriode) {
