...
// Récupération du Top 10 (identique à l'original)
$top_debiteurs = [];
$total_encours = 0;
try {
    $stmt = $pdo->prepare("
    ...
    foreach ($top_debiteurs as $d) $total_encours += $d['encours_total'];
...
foreach ($top_debiteurs as $d) {
    try {
        ...
        ");
        $stmt->execute([':client_id' => $d['client_id'], ':date_fin' => $date_fin_periode]);
        $details_par_debiteur[$d['client_id']] = $stmt->fetchAll();
    } catch (PDOException $e) { $details_par_debiteur[$d['client_id']] = []; }
}

// Portefeuille total et part du Top 10
...
$nb_superieur_10 = 0; $nb_superieur_25 = 0;
foreach ($top_debiteurs as $d) {
    $p = ($fonds_propres > 0) ? ($d['encours_total'] / $fonds_propres) * 100 : 0;
...
<head>
    ...
    <style>
        /* Styles identiques aux autres fichiers */
                ...
                <table>
                    ...
                    <tbody><?php foreach($top_debiteurs as $i=>$d): $pourc = ($fonds_propres>0)?($d['encours_total']/$fonds_propres)*100:0; $rowClass = $pourc>10?'danger-row':($d['has_impaye']?'warning-row':''); ?>
                        ...
                        <tr id="details-<?= $i ?>" class="hidden-row"><td colspan="9"><div style="padding:12px; background:#f8fafc;"><strong>Détail des prêts :</strong>
                            ...
                            <tbody><?php $det = $details_par_debiteur[$d['client_id']]??[]; if(empty($det)): ?>
                                <td><td colspan="7">Aucun prêt actif</td></tr><?php else: foreach($det as $p): ?>
                                <tr><td><?= htmlspecialchars($p['dossier_id']) ?></td><td><?= date('d/m/Y',strtotime($p['date_octroi'])) ?></td><td class="text-right"><?= number_format($p['montant_initial'],0,',',' ') ?></td><td class="text-right"><?= number_format($p['encours_restant'],0,',',' ') ?></td><td class="text-center"><?= $p['duree'] ?> mois</td><td><?= htmlspecialchars($p['objet']?:'-') ?></td><td class="text-center"><?= $p['nb_impayes'] ?></td></tr>
                                <?php endforeach; endif; ?>
                            </tbody></table></div></td></tr>
                    <?php endforeach; ?>
                    <tr class="total-row"><td colspan="6"><strong>TOTAL 10 PREMIERS DÉBITEURS</strong></td><td class="text-right"><strong><?= number_format($total_encours,0,',',' ') ?></strong></td><td class="text-right"><strong><?= number_format(($total_encours/max($fonds_propres,1))*100,2) ?>%</strong></td><td></td></tr>
                </tbody>
            </table>
            </div>
        </div>
...
<script>
    ...
    function exporterExcel() {
        ...
        <?php foreach($top_debiteurs as $i=>$d): $p = ($fonds_propres>0)?($d['encours_total']/$fonds_propres)*100:0; ?>
        data.push([<?= $i+1 ?>,'<?= addslashes($d['matricule']??'-') ?>','<?= addslashes($d['nom_complet']?:'N/A') ?>','<?= addslashes($d['categorie']?:'-') ?>','<?= addslashes($d['secteur_id']?:'-') ?>',<?= round($d['duree_moyenne']) ?>,<?= $d['encours_total'] ?>,<?= number_format($p,2,'.','') ?>]);
        <?php endforeach; ?>
