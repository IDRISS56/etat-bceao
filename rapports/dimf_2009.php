<?php
// DIMF_2009.php - Détail du compte 6221 (Personnel extérieur)
...
class PDF_DIMF extends FPDF {
    ...
    // Correction : utilisation de mb_convert_encoding (plus robuste)
...
// ============================================================
// TRAITEMENT DU FORMULAIRE D'ENREGISTREMENT (POST)
// ============================================================
$message = '';
$message_type = '';
...
foreach ($categories as $code => $lib) {
    ...
    if ($d) {
        $totaux['nationaux'] += (int)$d['nationaux'];
        $totaux['autre_umoa'] += (int)$d['autre_umoa'];
        $totaux['hors_umoa'] += (int)$d['hors_umoa'];
        $totaux['secteur_primaire'] += (int)$d['secteur_primaire'];
        $totaux['secteur_secondaire'] += (int)$d['secteur_secondaire'];
        $totaux['secteur_tertiaire'] += (int)$d['secteur_tertiaire'];
        $totaux['total_effectif'] += (int)$d['total_effectif'];
        $totaux['facturation'] += (float)$d['facturation'];
    }
}
...
<script>
    ...
    function exporterExcel() {
        ...
        const data = [['DIMF_2009 - PERSONNEL EXTÉRIEUR'],['Exercice','<?= $exercice ?>','Type','<?= $type_periode ?>'],[],['Catégorie','Nationaux','Autres UMOA','Hors UMOA','Primaire','Secondaire','Tertiaire','Total effectif','Facturation']];
        <?php foreach($categories as $code => $lib): 
            $d = $personnel_data[$code] ?? [
                'nationaux' => 0,
                'autre_umoa' => 0,
