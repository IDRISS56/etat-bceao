...
// FPDF intégré, gestion des filtres par POST, PDF en AJAX (téléchargement direct), Bootstrap
...
// A2H Dépôts à terme constitués (comptes_dat en cours)
$A2H_brut = 0;
...
$A2I_brut = 0; // Dépôts de garantie constitués (non géré)
$A2J_brut = 0; // Autres dépôts constitués (non géré)
$A2A_brut = $A2H_brut + $A2I_brut + $A2J_brut;

// A3A Comptes de prêts (A3B + A3C)
// A3B Prêts à moins d'un an (durée <= 12)
$A3B_brut = 0;
try {
    $stmt = $pdo->prepare("
...
// A60 Créances rattachées – on met 0 par défaut
$A60_brut = 0;
...
// Amortissement pour A70 (provisions sur créances)
$A70_amort = 0;
...
// A01
$A01_brut  = $A10_brut + $A2A_brut + $A3A_brut + $A60_brut + $A70_brut;
$A01_amort = 0 + 0 + 0 + 0 + $A70_amort;
...
$D60_brut = 0;
$D60_amort = 0;   // <- ajout important
$D71_brut = $D72_brut = $D73_brut = 0;
$D70_amort = 0;

// D10 - Prêts et titres subordonnés (Immobilisations financières)
try {
...
// D1A = D10 + D1E + D1L + D1S (on garde les autres à 0)
...
$D01_brut  = $D1A_brut + $D23_brut + $D30_brut + $D40_brut + $Z03_brut + $D50_brut + $D60_brut + $D70_brut;
$D01_amort = $D1A_amort + $D23_amort + $D30_amort + $D40_amort + $Z03_amort + $D50_amort + $D60_amort + $D70_amort;
...
$total_actif_amort = $A01_amort + $B01_amort + $C01_amort + $D01_amort + $E01_amort + 0;
...
// G01 Opérations avec les membres
$G10_net = 0;
...
if ($format === 'pdf') {
    ...
    // Lignes A01
    $pdf->TableRow($colsActif, ['','A01 - OPERATIONS DE TRESORERIE', PDF_DIMF::montant($A01_brut), PDF_DIMF::montant($A01_amort), PDF_DIMF::montant($A01_brut - $A01_amort)], 'subtotal');
    ...
    $pdf->TableRow($colsActif, ['A3B','Prets a moins d\'un an',             PDF_DIMF::montant($A3B_brut), '0', PDF_DIMF::montant($A3B_brut)]);
    $pdf->TableRow($colsActif, ['A3C','Prets a terme',                      PDF_DIMF::montant($A3C_brut), '0', PDF_DIMF::montant($A3C_brut)]);
    $pdf->TableRow($colsActif, ['A60','Creances rattachees',                PDF_DIMF::montant($A60_brut), '0', PDF_DIMF::montant($A60_brut)]);
    $pdf->TableRow($colsActif, ['A70','Prets en souffrance',                PDF_DIMF::montant($A70_brut), PDF_DIMF::montant($A70_amort), PDF_DIMF::montant($A70_brut - $A70_amort)]);
    $pdf->TableRow($colsActif, ['Z01','Prets immobilises',                  PDF_DIMF::montant($Z01_brut), '0', PDF_DIMF::montant($Z01_brut)]);
    ...
    $pdf->TableRow($colsActif, ['B65','Creances rattachees',       PDF_DIMF::montant($B65_brut), '0', PDF_DIMF::montant($B65_brut)]);
    ...
    $pdf->TableRow($colsActif, ['Z02','Credits immobilises',       PDF_DIMF::montant($Z02_brut), '0', PDF_DIMF::montant($Z02_brut)]);
    ...
    $pdf->TableRow($colsActif, ['C40','Debiteurs divers',          PDF_DIMF::montant($C40_brut), PDF_DIMF::montant($C40_amort), PDF_DIMF::montant($C40_brut - $C40_amort)]);
    $pdf->TableRow($colsActif, ['C55','Creances rattachees',       PDF_DIMF::montant($C55_brut), '0', PDF_DIMF::montant($C55_brut)]);
    $pdf->TableRow($colsActif, ['C56','Valeur a l\'encaissement',  PDF_DIMF::montant($C56_brut), '0', PDF_DIMF::montant($C56_brut)]);
    $pdf->TableRow($colsActif, ['C59','Valeur a rejeter',          PDF_DIMF::montant($C59_brut), '0', PDF_DIMF::montant($C59_brut)]);
    ...
    $pdf->TableRow($colsActif, ['C6G','Regul. actif',              PDF_DIMF::montant($C6G_brut), '0', PDF_DIMF::montant($C6G_brut)]);
    ...
    // Lignes D01
    $pdf->TableRow($colsActif, ['','D01 - VALEURS IMMOBILISEES', PDF_DIMF::montant($D01_brut), PDF_DIMF::montant($D01_amort), PDF_DIMF::montant($D01_brut - $D01_amort)], 'subtotal');
    $pdf->TableRow($colsActif, ['D1A','Immobilisations financieres', PDF_DIMF::montant($D1A_brut), PDF_DIMF::montant($D1A_amort), PDF_DIMF::montant($D1A_brut - $D1A_amort)]);
    $pdf->TableRow($colsActif, ['D10','Prets et titres subordonnes',PDF_DIMF::montant($D10_brut), PDF_DIMF::montant($D10_amort), PDF_DIMF::montant($D10_brut - $D10_amort)]);
    ...
    $pdf->TableRow($colsActif, ['D1S','Depots et cautionnements',   PDF_DIMF::montant($D1S_brut), PDF_DIMF::montant($D1S_amort), PDF_DIMF::montant($D1S_brut - $D1S_amort)]);
    ...
    $pdf->TableRow($colsActif, ['Z03','Acquises par realisation de garantie', PDF_DIMF::montant($Z03_brut), PDF_DIMF::montant($Z03_amort), PDF_DIMF::montant($Z03_brut - $Z03_amort)]);
    ...
    $pdf->TableRow($colsActif, ['D50','Credits bail et assimiles',  PDF_DIMF::montant($D50_brut), PDF_DIMF::montant($D50_amort), PDF_DIMF::montant($D50_brut - $D50_amort)]);
    $pdf->TableRow($colsActif, ['D51','Credits bail',               PDF_DIMF::montant($D51_brut), '0', PDF_DIMF::montant($D51_brut)]);
    ...
    $pdf->TableRow($colsActif, ['D60','Creances rattachees',        PDF_DIMF::montant($D60_brut), '0', PDF_DIMF::montant($D60_brut)]);
    $pdf->TableRow($colsActif, ['D70','Creances en souffrance',     PDF_DIMF::montant($D70_brut), PDF_DIMF::montant($D70_amort), PDF_DIMF::montant($D70_brut - $D70_amort)]);
    ...
    // Lignes E
    $pdf->TableRow($colsActif, ['','E01 - ACTIONNAIRES, ASSOCIES', PDF_DIMF::montant($E01_brut), '0', PDF_DIMF::montant($E01_brut)], 'subtotal');
    $pdf->TableRow($colsActif, ['E02','Capital non appele',         PDF_DIMF::montant($E02_brut), '0', PDF_DIMF::montant($E02_brut)]);
    $pdf->TableRow($colsActif, ['E03','Capital appele non verse',   PDF_DIMF::montant($E03_brut), '0', PDF_DIMF::montant($E03_brut)]);
    $pdf->TableRow($colsActif, ['E05','Excedent de charges sur produits', PDF_DIMF::montant($E05_brut), '0', PDF_DIMF::montant($E05_brut)]);
    ...
    $pdf->TableRow($colsPassif, ['F1A','Comptes ordinaires crediteurs', PDF_DIMF::montant($F1A_net)]);
    $pdf->TableRow($colsPassif, ['F2A','Autres comptes de depots crediteurs', PDF_DIMF::montant($F2A_net)]);
    $pdf->TableRow($colsPassif, ['F2B','Depots a terme recus', PDF_DIMF::montant($F2B_net)]);
    $pdf->TableRow($colsPassif, ['F2C','Depots de garantie recus', PDF_DIMF::montant($F2C_net)]);
    $pdf->TableRow($colsPassif, ['F2D','Autres depots recus', PDF_DIMF::montant($F2D_net)]);
    $pdf->TableRow($colsPassif, ['F3A','Comptes d\'emprunts', PDF_DIMF::montant($F3A_net)]);
    $pdf->TableRow($colsPassif, ['F3E','Emprunts a moins d\'un an', PDF_DIMF::montant($F3E_net)]);
    $pdf->TableRow($colsPassif, ['F3F','Emprunts a terme', PDF_DIMF::montant($F3F_net)]);
    $pdf->TableRow($colsPassif, ['F50','Autres sommes dues aux IF', PDF_DIMF::montant($F50_net)]);
    $pdf->TableRow($colsPassif, ['F55','Ressources affectees', PDF_DIMF::montant($F55_net)]);
    $pdf->TableRow($colsPassif, ['F60','Dettes rattachees', PDF_DIMF::montant($F60_net)]);
    ...
    $pdf->TableRow($colsPassif, ['G30','Autres depots de garantie recus', PDF_DIMF::montant($G30_net)]);
    $pdf->TableRow($colsPassif, ['G35','Autres depots recus', PDF_DIMF::montant($G35_net)]);
    ...
    $pdf->TableRow($colsPassif, ['G90','Dettes rattachees', PDF_DIMF::montant($G90_net)]);

    // H01
    $pdf->TableRow($colsPassif, ['','H01 - OPERATIONS SUR TITRES ET DIVERSES', PDF_DIMF::montant($H01_net)], 'subtotal');
    $pdf->TableRow($colsPassif, ['H10','Versements restant a effectuer', PDF_DIMF::montant($H10_net)]);
    $pdf->TableRow($colsPassif, ['H40','Crediteurs divers', PDF_DIMF::montant($H40_net)]);
    ...
    $pdf->TableRow($colsPassif, ['H6G','Comptes de regularisation passif', PDF_DIMF::montant($H6G_net)]);
    $pdf->TableRow($colsPassif, ['H6P','Comptes d\'attente passif', PDF_DIMF::montant($H6P_net)]);

    // K01
    $pdf->TableRow($colsPassif, ['','K01 - VERSEMENTS RESTANT A EFFECTUER', PDF_DIMF::montant($K01_net)], 'subtotal');
    $pdf->TableRow($colsPassif, ['K20','Titres de participation', PDF_DIMF::montant(0)]);

    // L01
    $pdf->TableRow($colsPassif, ['','L01 - PROVISIONS, FONDS PROPRES', PDF_DIMF::montant($L01_net)], 'subtotal');
    $pdf->TableRow($colsPassif, ['L10','Subventions d\'investissement', PDF_DIMF::montant($L10_net)]);
    $pdf->TableRow($colsPassif, ['L20','Fonds affectes', PDF_DIMF::montant($L20_net)]);
    ...
    $pdf->TableRow($colsPassif, ['L24','Fonds de securite', PDF_DIMF::montant($L24_net)]);
    $pdf->TableRow($colsPassif, ['L25','Autres fonds affectes', PDF_DIMF::montant($L25_net)]);
    $pdf->TableRow($colsPassif, ['L27','Fonds de credit', PDF_DIMF::montant($L27_net)]);
    ...
    $pdf->TableRow($colsPassif, ['L32','Provisions pour risque d\'execution', PDF_DIMF::montant($L32_net)]);
    $pdf->TableRow($colsPassif, ['L33','Autres provisions pour risques', PDF_DIMF::montant($L33_net)]);
    $pdf->TableRow($colsPassif, ['L35','Provisions reglementees', PDF_DIMF::montant($L35_net)]);
    $pdf->TableRow($colsPassif, ['L36','Provisions pour risques afferents', PDF_DIMF::montant($L36_net)]);
    $pdf->TableRow($colsPassif, ['L37','Provision speciale de reevaluation', PDF_DIMF::montant($L37_net)]);
    $pdf->TableRow($colsPassif, ['L41','Emprunts et titres subordonnes', PDF_DIMF::montant($L41_net)]);
    $pdf->TableRow($colsPassif, ['L43','Dettes rattachees aux emprunts', PDF_DIMF::montant($L43_net)]);
    $pdf->TableRow($colsPassif, ['L45','Fonds pour risques financiers', PDF_DIMF::montant($L45_net)]);
    $pdf->TableRow($colsPassif, ['L50','Primes liees au capital', PDF_DIMF::montant($L50_net)]);
    $pdf->TableRow($colsPassif, ['L55','Reserves', PDF_DIMF::montant($L55_net)]);
    $pdf->TableRow($colsPassif, ['L56','Reserve generale', PDF_DIMF::montant($L56_net)]);
    $pdf->TableRow($colsPassif, ['L57','Reserves facultatives', PDF_DIMF::montant($L57_net)]);
    $pdf->TableRow($colsPassif, ['L58','Autres reserves', PDF_DIMF::montant($L58_net)]);
    $pdf->TableRow($colsPassif, ['L60','Capital', PDF_DIMF::montant($L60_net)]);
    $pdf->TableRow($colsPassif, ['L61','Capital appele', PDF_DIMF::montant($L61_net)]);
    $pdf->TableRow($colsPassif, ['L62','Capital non appele', PDF_DIMF::montant($L62_net)]);
    $pdf->TableRow($colsPassif, ['L65','Fonds de dotation', PDF_DIMF::montant($L65_net)]);
    $pdf->TableRow($colsPassif, ['L70','Report a nouveau', PDF_DIMF::montant($L70_net)]);
    $pdf->TableRow($colsPassif, ['L75','Excedent des produits sur charges', PDF_DIMF::montant($L75_net)]);
    $pdf->TableRow($colsPassif, ['L80','Resultat de l\'exercice', PDF_DIMF::montant($L80_net)]);
    $pdf->TableRow($colsPassif, ['L81','Excedent en instance d\'approbation', PDF_DIMF::montant($L81_net)]);
    $pdf->TableRow($colsPassif, ['L82','Excedent de l\'exercice', PDF_DIMF::montant($L82_net)]);
    ...
    // On peut ajouter toutes les lignes du hors bilan (Z11 à Z19)
    $pdf->TableRow($colsHB, ['Z11','ENGAGEMENTS DE FINANCEMENT', PDF_DIMF::montant(0)], 'subtotal');
    $pdf->TableRow($colsHB, ['N1A','Engagements donnes en faveur des IF', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['N1H','Engagements recus des IF', PDF_DIMF::montant($N1H_net)]);
    $pdf->TableRow($colsHB, ['N1J','Engagements donnes en faveur des membres', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['N1K','Engagements recus des membres', PDF_DIMF::montant(0)]);
    ...
    $pdf->TableRow($colsHB, ['N2H','Recus des IF', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['N2J','D\'ordre des membres', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['N2M','Recus des membres', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Z13','ENGAGEMENTS SUR TITRES', PDF_DIMF::montant(0)], 'subtotal');
    $pdf->TableRow($colsHB, ['N3A','Titres a livrer', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['N3E','Titres a recevoir', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Z14','ENGAGEMENTS SUR OPERATIONS EN DEVISES', PDF_DIMF::montant(0)], 'subtotal');
    $pdf->TableRow($colsHB, ['Z15','Operations de change au comptant', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Z16','Prets ou emprunts en devises', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Z17','Operations de change a terme', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Z18','AUTRES ENGAGEMENTS', PDF_DIMF::montant(0)], 'subtotal');
    $pdf->TableRow($colsHB, ['Q1A','Engagements donnes', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Q1B','Engagements recus', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Z19','OPERATIONS POUR LE COMPTE DE TIERS', PDF_DIMF::montant(0)], 'subtotal');
    $pdf->TableRow($colsHB, ['Q1C','Valeurs a l\'encaissement non disponibles', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Q1F','Comptes exigibles apres encaissement', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Q1J','Suivi engagements financements consortiaux', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Q1K','Suivi engagements garanties consortiaux', PDF_DIMF::montant(0)]);
    $pdf->TableRow($colsHB, ['Q1L','Suivi credits consortiaux', PDF_DIMF::montant(0)]);
    ...
    // Sortie du PDF
    if ($ajax) {
        // Téléchargement direct sans nouvelle page
        $pdf->Output('D', 'DIMF_2000_Bilan_' . $exercice . '_' . $type_periode . '.pdf');
    } else {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media print { body { background: white; padding: 0; } .btn-group, .page-footer, #filtersCard { display: none !important; } .card { box-shadow: none; } }
...
<div class="dashboard">
    ...
    <!-- Formulaire principal pour les filtres (POST) -->
            ...
            <table>
                <thead>
                    <tr>
                        <th>CODE</th>
                        <th>POSTE ACTIF</th>
                        <th class="text-right">Brut (FCFA)</th>
                        <th class="text-right">Amort. (FCFA)</th>
                        <th class="text-right">Net (FCFA)</th>
                    </tr>
                </thead>
                <tbody>
                    <!-- A01 -->
                    <tr class="subtotal-row"><td colspan="2">A01 - OPÉRATIONS DE TRÉSORERIE</td>
                    ...
                    <tr><td>A71</td><td>Prêts en souffrance de 6 mois au plus</td><td class="text-right"><?= number_format($A71_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($A71_brut,0,',',' ') ?></td></tr>
                    <tr><td>A72</td><td>Prêts en souffrance de plus de 6 mois à 12 mois au plus</td><td class="text-right"><?= number_format($A72_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($A72_brut,0,',',' ') ?></td></tr>
                    <tr><td>A73</td><td>Prêts en souffrance de plus de 12 mois à 24 mois au plus</td><td class="text-right"><?= number_format($A73_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($A73_brut,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>B71</td><td>Crédits en souffrance de 6 mois au plus</td><td class="text-right"><?= number_format($B71_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($B71_brut,0,',',' ') ?></td></tr>
                    <tr><td>B72</td><td>Crédits en souffrance de plus de 6 mois à 12 mois au plus</td><td class="text-right"><?= number_format($B72_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($B72_brut,0,',',' ') ?></td></tr>
                    <tr><td>B73</td><td>Crédits en souffrance de plus de 12 mois à 24 mois au plus</td><td class="text-right"><?= number_format($B73_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($B73_brut,0,',',' ') ?></td></tr>

                    <!-- C01 -->
                    <tr class="subtotal-row"><td colspan="2">C01 - OPÉRATIONS SUR TITRES ET OPÉRATIONS DIVERSES</td>
                    ...
                    <tr><td>C34</td><td>Autres stocks et assimilés</td><td class="text-right"><?= number_format($C34_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($C34_brut,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>C56</td><td>Valeur à l'encaissement avec crédit immédiat</td><td class="text-right"><?= number_format($C56_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($C56_brut,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>C6C</td><td>Comptes de différence de conversion</td><td class="text-right"><?= number_format($C6C_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($C6C_brut,0,',',' ') ?></td></tr>
                    <tr><td>C6G</td><td>Comptes de régularisation actif</td><td class="text-right"><?= number_format($C6G_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($C6G_brut,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>C6R</td><td>Comptes d'attente actif</td><td class="text-right"><?= number_format($C6R_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($C6R_brut,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>Z03</td><td>Immobilisations acquises par réalisation de garantie</td><td class="text-right"><?= number_format($Z03_brut,0,',',' ') ?></td><td class="text-right"><?= number_format($Z03_amort,0,',',' ') ?></td><td class="text-right"><?= number_format($Z03_brut - $Z03_amort,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>D50</td><td>Crédits bail et opérations assimilées</td><td class="text-right"><?= number_format($D50_brut,0,',',' ') ?></td><td class="text-right"><?= number_format($D50_amort,0,',',' ') ?></td><td class="text-right"><?= number_format($D50_brut - $D50_amort,0,',',' ') ?></td></tr>
                    <tr><td>D51</td><td>Crédits bail</td><td class="text-right"><?= number_format($D51_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($D51_brut,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>D71</td><td>Créances en souffrance de 6 mois au plus</td><td class="text-right"><?= number_format($D71_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($D71_brut,0,',',' ') ?></td></tr>
                    <tr><td>D72</td><td>Créances en souffrance de plus de 6 mois à 12 mois au plus</td><td class="text-right"><?= number_format($D72_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($D72_brut,0,',',' ') ?></td></tr>
                    <tr><td>D73</td><td>Créances en souffrance de plus de 12 mois à 24 mois au plus</td><td class="text-right"><?= number_format($D73_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($D73_brut,0,',',' ') ?></td></tr>

                    <!-- E01 -->
                    <tr class="subtotal-row"><td colspan="2">E01 - ACTIONNAIRES, ASSOCIÉS OU MEMBRES</td>
                        <td class="text-right"><?= number_format($E01_brut,0,',',' ') ?></td>
                        <td class="text-right">0</td>
                        <td class="text-right"><?= number_format($E01_brut,0,',',' ') ?></td>
                    </tr>
                    <tr><td>E02</td><td>Actionnaires, associés ou membres, capital non appelé</td><td class="text-right"><?= number_format($E02_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($E02_brut,0,',',' ') ?></td></tr>
                    <tr><td>E03</td><td>Actionnaires, associés ou membres, capital appelé non versé</td><td class="text-right"><?= number_format($E03_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($E03_brut,0,',',' ') ?></td></tr>
                    <tr><td>E05</td><td>Excédent de charges sur les produits</td><td class="text-right"><?= number_format($E05_brut,0,',',' ') ?></td><td class="text-right">0</td><td class="text-right"><?= number_format($E05_brut,0,',',' ') ?></td></tr>
        ...
        <div class="table-wrapper">
            <table>
                ...
                <tbody>
                    ...
                    <tr><td>F50</td><td>Autres sommes dues aux institutions financières</td><td class="text-right"><?= number_format($F50_net,0,',',' ') ?></td></tr>
                    ...
                    <!-- H01 -->
                    <tr class="subtotal-row"><td colspan="2">H01 - OPÉRATIONS SUR TITRES ET OPÉRATIONS DIVERSES</td><td class="text-right"><?= number_format($H01_net,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>H6C</td><td>Comptes de différences de conversion</td><td class="text-right"><?= number_format($H6C_net,0,',',' ') ?></td></tr>
                    <tr><td>H6G</td><td>Comptes de régularisation-passif</td><td class="text-right"><?= number_format($H6G_net,0,',',' ') ?></td></tr>
                    <tr><td>H6P</td><td>Comptes d'attente Passif</td><td class="text-right"><?= number_format($H6P_net,0,',',' ') ?></td></tr>

                    <!-- K01 -->
                    <tr class="subtotal-row"><td colspan="2">K01 - VERSEMENTS RESTANT À EFFECTUER SUR IMMOBILISATIONS FINANCIÈRES</td><td class="text-right"><?= number_format($K01_net,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>L32</td><td>Provisions pour risque d'exécution des engagements par signature</td><td class="text-right"><?= number_format($L32_net,0,',',' ') ?></td></tr>
                    <tr><td>L33</td><td>Autres provisions pour risques et charges</td><td class="text-right"><?= number_format($L33_net,0,',',' ') ?></td></tr>
                    <tr><td>L35</td><td>Provisions réglementées</td><td class="text-right"><?= number_format($L35_net,0,',',' ') ?></td></tr>
                    <tr><td>L36</td><td>Provisions pour risques afférents aux opérations de crédits à moyen et long termes</td><td class="text-right"><?= number_format($L36_net,0,',',' ') ?></td></tr>
                    <tr><td>L37</td><td>Provision spéciale de réévaluation</td><td class="text-right"><?= number_format($L37_net,0,',',' ') ?></td></tr>
                    <tr><td>L41</td><td>Emprunts et titres émis subordonnés</td><td class="text-right"><?= number_format($L41_net,0,',',' ') ?></td></tr>
                    <tr><td>L43</td><td>Dettes rattachées aux emprunts et titres émis subordonnés</td><td class="text-right"><?= number_format($L43_net,0,',',' ') ?></td></tr>
                    <tr><td>L45</td><td>Fonds pour risques financiers généraux</td><td class="text-right"><?= number_format($L45_net,0,',',' ') ?></td></tr>
                    ...
                    <tr><td>L75</td><td>Excédent des produits sur les charges</td><td class="text-right"><?= number_format($L75_net,0,',',' ') ?></td></tr>
                    <tr><td>L80</td><td>Résultat de l'exercice</td><td class="text-right"><?= number_format($L80_net,0,',',' ') ?></td></tr>
                    <tr><td>L81</td><td>Excédent ou déficit en instance d'approbation</td><td class="text-right"><?= number_format($L81_net,0,',',' ') ?></td></tr>
                    <tr><td>L82</td><td>Excédent ou déficit de l'exercice</td><td class="text-right"><?= number_format($L82_net,0,',',' ') ?></td></tr>
        ...
        <div class="table-wrapper">
            <table>
                ...
                <tbody>
                    ...
                    <tr><td>N1A</td><td>Engagements donnés en faveur des institutions financières</td><td class="text-right">0</td></tr>
                    <tr><td>N1H</td><td>Engagements reçus des institutions financières</td><td class="text-right"><?= number_format($N1H_net,0,',',' ') ?></td></tr>
                    <tr><td>N1J</td><td>Engagements donnés en faveur des membres, bénéficiaires ou clients</td><td class="text-right">0</td></tr>
                    <tr><td>N1K</td><td>Engagements reçus des membres, bénéficiaires ou clients</td><td class="text-right">0</td></tr>

                    <tr class="subtotal-row"><td colspan="2">Z12 - ENGAGEMENTS DE GARANTIE</td><td class="text-right">0</td></tr>
                    <tr><td>N2A</td><td>D'ordre des institutions financières</td><td class="text-right">0</td></tr>
                    <tr><td>N2H</td><td>Reçus des institutions financières</td><td class="text-right">0</td></tr>
                    <tr><td>N2J</td><td>D'ordre des membres, bénéficiaires ou clients</td><td class="text-right">0</td></tr>
                    <tr><td>N2M</td><td>Reçus des membres, bénéficiaires ou clients</td><td class="text-right">0</td></tr>
                    ...
                    <tr class="subtotal-row"><td colspan="2">Z19 - OPÉRATIONS EFFECTUÉES POUR LE COMPTE DE TIERS</td><td class="text-right">0</td></tr>
                    <tr><td>Q1C</td><td>Valeurs à l'encaissement non disponibles</td><td class="text-right">0</td></tr>
                    <tr><td>Q1F</td><td>Comptes exigibles après encaissements</td><td class="text-right">0</td></tr>
                    <tr><td>Q1J</td><td>Comptes de suivi des engagements de financements consortiaux</td><td class="text-right">0</td></tr>
                    <tr><td>Q1K</td><td>Comptes de suivi des engagements de garanties consortiaux</td><td class="text-right">0</td></tr>
                    <tr><td>Q1L</td><td>Comptes de suivi des crédits consortiaux</td><td class="text-right">0</td></tr>
...
<script>
    ...
    async function exporterPDF() {
        ...
        try {
            ...
            link.download = 'DIMF_2000_Bilan_<?= $exercice ?>_<?= $type_periode ?>.pdf';
    ...
    function exporterExcel() {
        ...
        const dataActif = [
            ...
            ['A01','OPERATIONS DE TRESORERIE',<?= $A01_brut ?>,<?= $A01_amort ?>,<?= $A01_brut - $A01_amort ?>],
            ...
            ['A3B','Prets a moins d\'un an',<?= $A3B_brut ?>,0,<?= $A3B_brut ?>],
            ['A3C','Prets a terme',<?= $A3C_brut ?>,0,<?= $A3C_brut ?>],
            ['A60','Creances rattachees',<?= $A60_brut ?>,0,<?= $A60_brut ?>],
            ['A70','Prets en souffrance',<?= $A70_brut ?>,<?= $A70_amort ?>,<?= $A70_brut - $A70_amort ?>],
            ['Z01','Prets immobilises',<?= $Z01_brut ?>,0,<?= $Z01_brut ?>],
            ...
            ['B65','Creances rattachees',<?= $B65_brut ?>,0,<?= $B65_brut ?>],
            ['B70','Credits en souffrance',<?= $B70_brut ?>,<?= $B70_amort ?>,<?= $B70_brut - $B70_amort ?>],
            ['Z02','Credits immobilises',<?= $Z02_brut ?>,0,<?= $Z02_brut ?>],
            ...
            ['C40','Debiteurs divers',<?= $C40_brut ?>,<?= $C40_amort ?>,<?= $C40_brut - $C40_amort ?>],
            ['C55','Creances rattachees',<?= $C55_brut ?>,0,<?= $C55_brut ?>],
            ['C56','Valeur a l\'encaissement',<?= $C56_brut ?>,0,<?= $C56_brut ?>],
            ['C59','Valeur a rejeter',<?= $C59_brut ?>,0,<?= $C59_brut ?>],
            ...
            ['C6G','Regul. actif',<?= $C6G_brut ?>,0,<?= $C6G_brut ?>],
            ['C6Q','Comptes transitoires',<?= $C6Q_brut ?>,0,<?= $C6Q_brut ?>],
            ['C6R','Comptes d\'attente',<?= $C6R_brut ?>,0,<?= $C6R_brut ?>],
            ['D01','VALEURS IMMOBILISEES',<?= $D01_brut ?>,<?= $D01_amort ?>,<?= $D01_brut - $D01_amort ?>],
            ['D1A','Immobilisations financieres',<?= $D1A_brut ?>,<?= $D1A_amort ?>,<?= $D1A_brut - $D1A_amort ?>],
            ['D10','Prets et titres subordonnes',<?= $D10_brut ?>,<?= $D10_amort ?>,<?= $D10_brut - $D10_amort ?>],
            ...
            ['D1S','Depots et cautionnements',<?= $D1S_brut ?>,<?= $D1S_amort ?>,<?= $D1S_brut - $D1S_amort ?>],
            ...
            ['Z03','Acquises par realisation de garantie',<?= $Z03_brut ?>,<?= $Z03_amort ?>,<?= $Z03_brut - $Z03_amort ?>],
            ['D46','Incorporelles',<?= $D46_brut ?>,<?= $D46_amort ?>,<?= $D46_brut - $D46_amort ?>],
            ['D47','Corporelles',<?= $D47_brut ?>,<?= $D47_amort ?>,<?= $D47_brut - $D47_amort ?>],
            ['D50','Credits bail et assimiles',<?= $D50_brut ?>,<?= $D50_amort ?>,<?= $D50_brut - $D50_amort ?>],
            ['D51','Credits bail',<?= $D51_brut ?>,0,<?= $D51_brut ?>],
            ['D52','L.o.a.',<?= $D52_brut ?>,0,<?= $D52_brut ?>],
            ['D53','Location vente',<?= $D53_brut ?>,0,<?= $D53_brut ?>],
            ['D60','Creances rattachees',<?= $D60_brut ?>,0,<?= $D60_brut ?>],
            ['D70','Creances en souffrance',<?= $D70_brut ?>,<?= $D70_amort ?>,<?= $D70_brut - $D70_amort ?>],
            ...
            ['E01','ACTIONNAIRES, ASSOCIES',<?= $E01_brut ?>,0,<?= $E01_brut ?>],
            ['E02','Capital non appele',<?= $E02_brut ?>,0,<?= $E02_brut ?>],
            ['E03','Capital appele non verse',<?= $E03_brut ?>,0,<?= $E03_brut ?>],
            ['E05','Excedent de charges sur produits',<?= $E05_brut ?>,0,<?= $E05_brut ?>],
        ...
        const dataPassif = [
            ...
            ['F01','OPERATIONS DE TRESORERIE',<?= $F01_net ?>],
            ['F1A','Comptes ordinaires crediteurs',<?= $F1A_net ?>],
            ['F2A','Autres comptes de depots crediteurs',<?= $F2A_net ?>],
            ['F2B','Depots a terme recus',<?= $F2B_net ?>],
            ['F2C','Depots de garantie recus',<?= $F2C_net ?>],
            ['F2D','Autres depots recus',<?= $F2D_net ?>],
            ['F3A','Comptes d\'emprunts',<?= $F3A_net ?>],
            ['F3E','Emprunts a moins d\'un an',<?= $F3E_net ?>],
            ['F3F','Emprunts a terme',<?= $F3F_net ?>],
            ['F50','Autres sommes dues aux IF',<?= $F50_net ?>],
            ['F55','Ressources affectees',<?= $F55_net ?>],
            ['F60','Dettes rattachees',<?= $F60_net ?>],
            ...
            ['G30','Autres depots de garantie recus',<?= $G30_net ?>],
            ['G35','Autres depots recus',<?= $G35_net ?>],
            ['G60','Emprunts',<?= $G60_net ?>],
            ['G70','Autres sommes dues',<?= $G70_net ?>],
            ['G90','Dettes rattachees',<?= $G90_net ?>],
            ['H01','OPERATIONS SUR TITRES ET DIVERSES',<?= $H01_net ?>],
            ['H10','Versements restant a effectuer',<?= $H10_net ?>],
            ['H40','Crediteurs divers',<?= $H40_net ?>],
            ['H6A','Comptes d\'ordre et divers',<?= $H6A_net ?>],
            ['H6B','Comptes de liaison',<?= $H6B_net ?>],
            ['H6C','Diff. de conversion',<?= $H6C_net ?>],
            ['H6G','Regul. passif',<?= $H6G_net ?>],
            ['H6P','Comptes d\'attente passif',<?= $H6P_net ?>],
            ['K01','VERSEMENTS RESTANT A EFFECTUER',<?= $K01_net ?>],
            ['K20','Titres de participation',0],
            ['L01','PROVISIONS, FONDS PROPRES',<?= $L01_net ?>],
            ['L10','Subventions d\'investissement',<?= $L10_net ?>],
            ['L20','Fonds affectes',<?= $L20_net ?>],
            ['L21','Fonds de garantie',<?= $L21_net ?>],
            ['L22','Fonds d\'assurance',<?= $L22_net ?>],
            ['L23','Fonds de bonification',<?= $L23_net ?>],
            ['L24','Fonds de securite',<?= $L24_net ?>],
            ['L25','Autres fonds affectes',<?= $L25_net ?>],
            ['L27','Fonds de credit',<?= $L27_net ?>],
            ['L30','Provisions pour risques et charges',<?= $L30_net ?>],
            ['L31','Provisions pour charges de retraite',<?= $L31_net ?>],
            ['L32','Provisions pour risque d\'execution',<?= $L32_net ?>],
            ['L33','Autres provisions pour risques',<?= $L33_net ?>],
            ['L35','Provisions reglementees',<?= $L35_net ?>],
            ['L36','Provisions pour risques afferents',<?= $L36_net ?>],
            ['L37','Provision speciale de reevaluation',<?= $L37_net ?>],
            ['L41','Emprunts et titres subordonnes',<?= $L41_net ?>],
            ['L43','Dettes rattachees aux emprunts',<?= $L43_net ?>],
            ['L45','Fonds pour risques financiers',<?= $L45_net ?>],
            ['L50','Primes liees au capital',<?= $L50_net ?>],
            ['L55','Reserves',<?= $L55_net ?>],
            ['L56','Reserve generale',<?= $L56_net ?>],
            ['L57','Reserves facultatives',<?= $L57_net ?>],
            ['L58','Autres reserves',<?= $L58_net ?>],
            ['L60','Capital',<?= $L60_net ?>],
            ['L61','Capital appele',<?= $L61_net ?>],
            ['L62','Capital non appele',<?= $L62_net ?>],
            ['L65','Fonds de dotation',<?= $L65_net ?>],
            ['L70','Report a nouveau',<?= $L70_net ?>],
            ['L75','Excedent des produits sur charges',<?= $L75_net ?>],
            ['L80','Resultat de l\'exercice',<?= $L80_net ?>],
            ['L81','Excedent en instance d\'approbation',<?= $L81_net ?>],
            ['L82','Excedent de l\'exercice',<?= $L82_net ?>],
        ...
        const dataHB = [
            ...
            ['N1A','Engagements donnes en faveur des IF',0],
            ['N1H','Engagements recus des IF',<?= $N1H_net ?>],
            ['N1J','Engagements donnes en faveur des membres',0],
            ['N1K','Engagements recus des membres',0],
            ['Z12','ENGAGEMENTS DE GARANTIE',0],
            ['N2A','D\'ordre des IF',0],
            ['N2H','Recus des IF',0],
            ['N2J','D\'ordre des membres',0],
            ['N2M','Recus des membres',0],
            ['Z13','ENGAGEMENTS SUR TITRES',0],
            ['N3A','Titres a livrer',0],
            ['N3E','Titres a recevoir',0],
            ['Z14','ENGAGEMENTS SUR OPERATIONS EN DEVISES',0],
            ['Z15','Operations de change au comptant',0],
            ['Z16','Prets ou emprunts en devises',0],
            ['Z17','Operations de change a terme',0],
            ['Z18','AUTRES ENGAGEMENTS',0],
            ['Q1A','Engagements donnes',0],
            ['Q1B','Engagements recus',0],
            ['Z19','OPERATIONS POUR LE COMPTE DE TIERS',0],
            ['Q1C','Valeurs a l\'encaissement non disponibles',0],
            ['Q1F','Comptes exigibles apres encaissement',0],
            ['Q1J','Suivi engagements financements consortiaux',0],
            ['Q1K','Suivi engagements garanties consortiaux',0],
            ['Q1L','Suivi credits consortiaux',0],
            ['Q1M','Credits distribues pour compte de tiers',<?= $Q1M_net ?>],
