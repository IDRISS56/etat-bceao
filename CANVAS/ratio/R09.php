...
// Z53 (participations dans d'autres SFD) – déjà déduites du numérateur, mais on les déduit aussi des fonds propres ? Selon le canevas, Z53 est à déduire des fonds propres.
try {
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    ...
    $pdf->codeDimf = 'R09'; $pdf->titreDimf = 'LIMITATION DES PRISES DE PARTICIPATION';
    ...
    $pdf->TableHeader($cols);
    foreach ($lignesPositifs as $r) { $pdf->TableRow($cols, [$r['code'], $r['lib'], PDF_DIMF::montant($r['montant'])]); }
    $pdf->TableRow($cols, ['', 'TOTAL ÉLÉMENTS POSITIFS', PDF_DIMF::montant($fondsPropPositifs)], 'subtotal');
    foreach ($lignesDeductions as $r) { $pdf->TableRow($cols, [$r['code'], $r['lib'], PDF_DIMF::montant($r['montant'])]); }
    ...
    $pdf->Ln(5);
    $pdf->SetFont('Arial','B',10); $pdf->Cell(0,7,PDF_DIMF::u("RATIO R09 = A / B = ".number_format($pourcentage,2)."%"),0,1);
    $pdf->SetFont('Arial','',9); $pdf->MultiCell(0,5,PDF_DIMF::u("Norme BCEAO : 0% ≤ Ratio ≤ 25%\nConformité : ".$conformite));
    $pdf->Output('I','R09_'.$exercice.'_'.$type_periode.'.pdf'); exit;
}
...
<script>
    ...
    function appliquerFiltres() {
        let url = 'R09.php?exercice=' + document.getElementById('exerciceSelect').value + '&type_periode=' + document.getElementById('typePeriodeSelect').value;
